...
class CreateMap extends Area {
	...
	createMap() {
		...
		if (level > 2) {
			...
			for (var i = 0; i < roomCount; i++) {
				...
				this.createRoomEnemy(roomX, roomY)
	...
	createRoomEnemy(x, y) {
		...
		do {
			...
			if (this.map[y][x] == 0) {
				const poe = dataArea[this.areaCode].bossMonster;
				let index = Math.floor(Math.random() * poe.length);
				this.map[y][x] = 5;
				this.roomEnemyData[createCount] = {
					x: x, y: y,
					dataIndex: index,
	...
	createFinishMap(x, y, lengthX, lengthY, type, diraction) {
		...
		if (this.party.x === this.exit.x && this.party.y === this.exit.y) {
			...
			if (answer) {
				...
				const characterNames = Object.getOwnPropertyNames(playerGroup) 
				for(let i =0 ; i < characterNames.length; i++){
					playerGroup[characterNames[i]].activeType = 'None'
				 }
			}
		}
	}
	...
	checkBossEnemy(x, y) {
		const pOEData = Object.getOwnPropertyNames(this.roomEnemyData);
		const length = pOEData.length
		for (let i = 0; i < length; i++) {
			if (this.roomEnemyData[pOEData[i]].x == x && this.roomEnemyData[pOEData[i]].y == y) {
				new AddLog([{ text: 'Position of Enemy Join' },], 'Map')
				battleCheck = 0;
				this.roomEnemyCount = pOEData[i];
				const power = this.roomEnemyData[pOEData[i]].powerValue
				enemyGroup = new EnemyGroup(this.areaCode, this.areaLevel * power, 'position', this.roomEnemyData[pOEData[i]].dataIndex).returnGroup
				new BattleData
...
class InMapParty {
	...
	createPlayerGroup() {
		...
		const groupName = new DOMSearch('id', 'GroupNameMain').value
		if (groupName != 'None') {
			const listData = groupList[groupName]
			const cList = Object.getOwnPropertyNames(listData.characterList);
			for (let i = 0; i < cList.length; i++) {
				if (character[cList[i]].status == "Death" || character[cList[i]].activeType != 'None') {
					this.createTrigger = false;
				}
			}
			if (this.createTrigger) {
				for (let i = 0; i < cList.length; i++) {
					player[cList[i]] = character[cList[i]]

					player[cList[i]].activeType = "InMap"
				}
			}
		}
		else {
			...
			for (let i = 0; i < length; i++) {
				if (character[characterName[i]].status == "Death" || character[characterName[i]].activeType != 'None') {
					this.createTrigger = false;
				}
			}
			if (this.createTrigger) {
			for (let i = 0; i < length; i++) {
				if (character[characterName[i]].selected === 1) {
					...
					player[characterName[i]].activeType= "InMap"
				}
		...
		if ( this.createTrigger && checkLength <= 5 && checkLength >= 1) {
			this.character = player
		}
		else if(this.createTrigger){
			console.log('Error')
			let c = 0;
			for (let i = 0; c < 5; i++) {
				if (character[characterName[i]].status == "Death" || character[characterName[i]].activeType != 'None') {
			...
			new AddLog([{text:"Group Create Fail"}],'System');
		}
	}
	createPartyDesk() {

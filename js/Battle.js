			...
			for (var j = 0; j < nowTargetLength; j++) {
				...
				if (hitCheck === 1) {
					...
					for (var k = 0; k < healthTypeLength; k++) {
						value = new SkillValue(performer, hitTarget, action)
				...
				new AddEffect(performer, hitTarget, action, value.value)
...
class Effect {
	...
	checkEffectSuffix(effectType) {
		...
		else if (effectType.indexOf('Call') != -1) {
			suffix = 'Call'
		}
		return suffix
	}
	checkEffectTarget(effectTarget) {
		...
		switch (effectTarget) {
			...
			case 'DefPhyPer':
				...
				break;
			case 'Target' :
				changeTarget = ['dataActiveCharacter']
				break;
		}
		return changeTarget
	}
...
class AddEffect {
	constructor(performer, target, action, damage) {
	...
	activeAddEffect(type) {
		...
		for (let i = 0; i < length; i++) {
			...
			const limit = effect.limit
			const name = effect.name
			if(name.indexOf('Damage')!= -1){
				value = Math.ceil(this.damage / 100 * value);
			}
			this.applyEffect(type, address, suffix, value)
		...
		switch (suffix) {
			...
			case 'Call':
				for (let i = 0; i < value.length; i++) {
					let summon = new Summon(value[i], 1, 0, this.target.ally)
					new Option(summon, 'battle')
					let inSummon = new BattleCharacter(summon)
					new StatusView(inSummon, 'Battle')
					dataActiveCharacter[inSummon.code] = inSummon;
				}
				break;
		}
	}
	...
	checkDeathCharacter() {
		...
		for (var i = 0; i < length; i++) {
			if (character[checkData[i]].health.hp <= 0 && character[checkData[i]].status != 'Death') {
				...
				if(checkData[i].indexOf('S') != -1){
					this.deleteData.push(checkData[i])
	...
	checkDelete(){
		if(this.deleteData.length > 0){
			for(let i = 0 ; i < this.deleteData.length; i++){
				new DOMSearch('id', 'Battle' + this.deleteData[i] + 'Border').remove();
				delete dataActiveCharacter[this.deleteData[i]]
			}
		}
	}
}
